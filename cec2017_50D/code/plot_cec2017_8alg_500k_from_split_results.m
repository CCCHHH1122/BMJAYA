...
% =====================================================================
% Plot CEC2017 convergence curves from the split MaxFEs=500000 result
% files. The output follows the same visual style used for the CEC2022
% D10/D20 convergence figures.
% =====================================================================
...
primary_result = fullfile(this_dir, 'results_cec2017', ...
    'results_CEC2017_4alg_D50_N50_FEs500000_Runs30.mat');
secondary_result = fullfile(parent_dir, ...
    'CEC2017_50D_BMJAYA_package - 副本', 'results_cec2017', ...
    'results_CEC2017_5alg_D50_N50_FEs500000_Runs30.mat');

output_dir = fullfile(this_dir, 'results_cec2017');
combined_mat = fullfile(output_dir, ...
    'results_CEC2017_8alg_D50_N50_FEs500000_Runs30_combined.mat');

target_alg_names = {'PSO','GWO','WSO','DE','JAYA','SBOA','DOA','BMJAYA'};
preferred_source = containers.Map( ...
    {'PSO','GWO','WSO','DE','JAYA','SBOA','DOA','BMJAYA'}, ...
...
%% ============================= LOAD ==================================
assert(exist(primary_result, 'file') == 2, ...
    'Primary result file not found: %s', primary_result);
assert(exist(secondary_result, 'file') == 2, ...
    'Secondary result file not found: %s', secondary_result);

A = load(primary_result, 'config', 'curve_error_sampled');
B = load(secondary_result, 'config', 'curve_error_sampled');

validate_compatible_configs(A.config, B.config);

config = A.config;
config.alg_names = target_alg_names;
config.source_files = {primary_result; secondary_result};
config.source_note = ['CEC2017 MaxFEs=500000 curves merged from split ', ...
    '4-algorithm and 5-algorithm result files.'];

func_list = config.func_list;
fes_show = config.fes_show;
nFunc = numel(func_list);
nAlg = numel(target_alg_names);
nRuns = size(A.curve_error_sampled, 1);
nPts = numel(fes_show);

curve_error_sampled = nan(nRuns, nFunc, nAlg, nPts, 'single');
source_used = cell(1, nAlg);

for alg_id = 1:nAlg
    alg_name = target_alg_names{alg_id};
    source_name = preferred_source(alg_name);

    switch source_name
        case 'primary'
            [curve_error_sampled(:, :, alg_id, :), source_used{alg_id}] = ...
                extract_algorithm_curves(A, alg_name, 'primary');
        case 'secondary'
            [curve_error_sampled(:, :, alg_id, :), source_used{alg_id}] = ...
                extract_algorithm_curves(B, alg_name, 'secondary');
        otherwise
            error('Unknown source "%s" for algorithm %s.', source_name, alg_name);
    end
end

save(combined_mat, 'config', 'curve_error_sampled', 'source_used', '-v7.3');
...
[colors, line_styles] = fixed_algorithm_styles(target_alg_names);

bmjaya_idx = find(strcmpi(target_alg_names, 'BMJAYA'), 1);
plot_order = [setdiff(1:nAlg, bmjaya_idx, 'stable'), bmjaya_idx];

fig = figure('Color','w','Position',[120 25 1120 1120]);
...
for p = 1:nFunc
    ...
    for order_id = 1:nAlg
        ...
        semilogy(ax, fes_show, med_curve, line_styles{alg_id}, ...
            ...
            'MarkerSize', 5, ...
            'Color', colors(alg_id, :), ...
            'DisplayName', target_alg_names{alg_id});
    end

    finite_y = med_curves(isfinite(med_curves) & med_curves >= 0);
    if isempty(finite_y)
        y_high = 1;
    else
        y_high = max(finite_y);
    ...
    if show_inset && func_list(p) ~= 4
        add_late_stage_inset(ax, fes_show, med_curves, target_alg_names, ...
            colors, plot_order, config.MaxFEs, inset_fe_fraction);
    end

    legend(ax, 'Location', 'northeast', 'FontSize', 7, 'Box', 'on');
end

if export_png
    exportgraphics(fig, fullfile(output_dir, ...
        'CEC2017_8alg_convergence_D50_MaxFEs500000.png'), ...
        'Resolution', 600);
end

if export_pdf
    exportgraphics(fig, fullfile(output_dir, ...
        'CEC2017_8alg_convergence_D50_MaxFEs500000.pdf'), ...
        'ContentType', 'vector');
end

fprintf('Combined MAT saved to: %s\n', combined_mat);
fprintf('Figure saved to: %s\n', fullfile(output_dir, ...
    'CEC2017_8alg_convergence_D50_MaxFEs500000.png'));

%% ========================== LOCAL FUNCTIONS ==========================
function validate_compatible_configs(config_a, config_b)
same_functions = isequal(config_a.func_list, config_b.func_list);
same_fes = isequal(config_a.fes_show, config_b.fes_show);
same_dimension = config_a.D == config_b.D;
same_population = config_a.N == config_b.N;
same_runs = config_a.Runs == config_b.Runs;
same_budget = config_a.MaxFEs == config_b.MaxFEs;

assert(same_functions, 'The two files use different CEC2017 functions.');
assert(same_fes, 'The two files use different sampled FEs.');
assert(same_dimension, 'The two files use different dimensions.');
assert(same_population, 'The two files use different population sizes.');
assert(same_runs, 'The two files use different run counts.');
assert(same_budget, 'The two files use different MaxFEs.');
end

function [curves, source_label] = extract_algorithm_curves(S, alg_name, source_label)
alg_names = S.config.alg_names(:)';
idx = find(strcmpi(alg_names, alg_name), 1);
assert(~isempty(idx), 'Algorithm %s was not found in %s result file.', ...
    alg_name, source_label);
curves = S.curve_error_sampled(:, :, idx, :);
end
...
function add_late_stage_inset(parent_ax, fes_show, med_curves, alg_names, ...
    colors, plot_order, MaxFEs, inset_fe_fraction)
fig = ancestor(parent_ax, 'figure');
parent_pos = parent_ax.Position;
rel_pos = [0.31, 0.51, 0.36, 0.30];
inset_pos = [ ...
...
if ~isempty(y_local)
    ...
    y_span = y_max - y_min;
    y_zoom_lim = [max(0, y_min - 0.10 * y_span), y_max + 0.10 * y_span];
